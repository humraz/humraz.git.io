...
export default function Bookmarks()
{
    ...
    useEffect(()=>{
        try{
            ...
            bookmarks.map(item=> console.log(item[0].title))
        }
        catch(error)
        {
    ...
    function handlesearch(e)
    {
        ...
        bookmarks.map(item=> console.log(item[0].title.substring(0,sear.length)))
        console.log(sear)
        setfilterdata([])
        if(sear.length>0)
        {
            setfilterdata(bookmarks.filter(e => e[0].title.substring(0,sear.length).toLowerCase()===sear.toLowerCase()))
        }
        setloading(!loading)
    }
    ...
    {
      setbookmarks([...bookmarks,(bookmarks.filter(e => e.title.toLowerCase()===title.toLowerCase()))])
      localStorage.clear()
                ...
                <div className="movies">
                    {
                        filterdata && filterdata.length > 0 ? filterdata.map(item => 
                            <Moviecard handlebookmark={handlebookmark} movieimage={item[0].thumbnail.regular.small} title={item[0].title} year={item[0].year} type={item[0].category} rating={item[0].rating}/>) : 
                
                        bookmarks && bookmarks.length > 0 ? bookmarks.map(item => 
                            <Moviecard handlebookmark={handlebookmark} movieimage={item[0].thumbnail.regular.small} title={item[0].title} year={item[0].year} type={item[0].category} rating={item[0].rating}/>) : null }
                </div>
            </div>
        </div>
    );
